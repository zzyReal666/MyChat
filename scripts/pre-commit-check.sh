...
if black --check --diff backend/; then
    ...
    log_error "Python 代码格式化检查失败，请运行: black backend/    exit 1
fi
...
if flake8-format=default --statistics backend/; then
    ...
    log_error "Python 代码风格检查失败    exit 1
fi
...
if ! command -v mypy &> /dev/null; then
    log_warning "mypy 未安装，跳过类型检查    log_info 建议安装: pip install mypy"
else
    log_info "运行 mypy 类型检查..."
    if mypy --ignore-missing-imports backend/; then
        log_success "Python 类型检查通过  else
...
if ! command -v isort &> /dev/null; then
    log_warning "isort 未安装，跳过导入排序检查    log_info 建议安装: pip install isort"
else
    log_info "运行 isort 检查..."
    if isort --check-only --diff backend/; then
        log_success "Python 导入排序检查通过  else
...
if ! command -v bandit &> /dev/null; then
    log_warning "bandit 未安装，跳过安全扫描    log_info 建议安装: pip install bandit"
else
    log_info "运行 bandit 安全扫描..."
    if bandit -r backend/ -f json -o backend/bandit-report.json; then
        log_success "安全扫描完成，报告已保存到 backend/bandit-report.json  else
...
if ! command -v safety &> /dev/null; then
    log_warning "safety 未安装，跳过依赖安全检查    log_info 建议安装:pip install safety"
else
    log_info "运行safety 依赖安全检查..."
    if safety check --json --output backend/safety-report.json; then
        log_success "依赖安全检查完成  else
...
if [ -d "frontend" ]; then
    ...
    if [ ! -d "node_modules" ]; then
        log_warning "前端依赖未安装，请运行: cd frontend && npm install    else
        # 检查 ESLint
...
if ! command -v pytest &> /dev/null; then
    log_warning "pytest 未安装，跳过测试    log_info 建议安装:pip install pytest pytest-covelse
