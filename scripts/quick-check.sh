...
# 2. Python 代码格式化检查
echo -en "${YELLOW}2. 检查 Python 代码格式化...${NC}"
if black --check backend/; then
    echo -e "${GREEN}✓ Black 检查通过${NC}"
else
    echo -e "${RED}✗ Black 检查失败，请运行: black backend/${NC}"
    exit 1
fi

# 3. Python 代码风格检查
echo -en "${YELLOW}3. 检查 Python 代码风格...${NC}"
...
# 4. 导入排序检查
echo -en "${YELLOW}4. 检查导入排序...${NC}"
if isort --check-only backend/; then
    echo -e "${GREEN}✓ Isort 检查通过${NC}"
else
    echo -e "${RED}✗ Isort 检查失败，请运行: isort backend/${NC}"
    exit 1
fi

# 5. 前端检查（如果存在）
if [ -d "frontend" ]; then
    echo -en "${YELLOW}5. 检查前端代码...${NC}"
    cd frontend
